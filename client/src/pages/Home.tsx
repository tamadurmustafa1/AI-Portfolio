  ...
  return (
    <section className="work-section" id="work">
      ...
      <div className="works-orbit" aria-label={lang === "ar" ? "فئات الأعمال" : "Work categories"}>
        ...
        <div className="orbit-items">{sections.map((section, index) => <a className={`orbit-item orbit-item-${index + 1} accent-${section.accent}`} key={section.id} href={`#${section.id}`} aria-label={`${L(lang, section.title)} — ${lang === "ar" ? "افتح القسم" : "Open category"}`}><span className="orbit-object"><span className="orbit-object-top" /><span className="orbit-object-icon">{categoryIcon(section.id)}</span><strong>{L(lang, section.title)}</strong><small>{String(section.projects.length).padStart(2, "0")} {lang === "ar" ? "أعمال" : "WORKS"}</small><i className="orbit-status" /></span></a>)}</div>
